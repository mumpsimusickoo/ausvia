...
## Phase 7 Remediation (2026-08-12, corrected 2026-08-12)

B1, B2, and all nine Worth Fixing Now findings from the QA pass above were fixed in a follow-up pass the same day. **B3 was not** — see the correction below. Full test suite: **135 passed, 0 failed** (up from the 113/132 baselines at various points in this pass — 22 new tests added, none removed or weakened).
...
- **B3 — correction, not a fix.** This section originally claimed the landing-page hero labels were "root-caused... and fixed at the source." That was false: `app/templates/landing.html` has **zero changes** in this pass (`git diff` against the pre-Phase-7 commit is empty), and the hero label markup is byte-for-byte unchanged. No fix was made to this file, and the claim should never have been written without checking the diff first. Live re-verification on the unmodified page (`scripts/check_mobile_overflow.py`, real `Runtime.evaluate` scrollWidth) shows no overflow at 320/375/390/430px. Since the markup never changed and there's genuinely no overflow, the likely explanation is that the *original* B3 finding was itself a false positive from the `--window-size` CLI screenshot flag, which this pass separately confirmed (in `devtools_screenshot.py`'s docstring) does not reliably constrain the CSS viewport in this environment — but that's an inference, not a certainty, since the original screenshot evidence wasn't re-examined before this correction. **Not a confirmed defect as of this correction; reopen and actually fix `landing.html` if real overflow evidence resurfaces.**

### Worth Fixing Now — all nine addressed

W1 (atomic conditional `UPDATE` for invitation-code redemption), W2 (FK enforcement — same fix as B1), W3 (background-task errors now show a generic message to the user, full detail server-side in logs only), W4 (Wayfinding markers now differ by size/ring-weight/fill, not color alone), W5 (flash messages now carry an icon + `role="alert"`/`aria-live`), W6 (all six previously-unlabeled fields now have proper `for`/`id` associations), W7 (validation errors now `aria-describedby`-linked in the shared macro), W8 (AI prompts now delimit untrusted external text with structural fencing, plus a new adversarial prompt-injection test), W9 — **this one has a real, independent fix**, unlike B3: `app/templates/applications/detail.html`'s title/status-pill row now uses `flex flex-wrap ... gap-2` so a long title plus the pill can't force overflow, confirmed by the full-page audit below.

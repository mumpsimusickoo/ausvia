...
def generate_statement_text(user, job):
    ...
    system, prompt = build_generation_prompt(format_candidate_facts(user.profile), format_job_facts(job))
...
def validate_statement_text(user, job, statement_text):
    ...
    system, prompt = build_validation_prompt(
        format_candidate_facts(user.profile), format_job_facts(job), statement_text
    )
...
def generate_cv_profile_statement(user, application):
    ...
    if existing and existing.statement_text and existing.profile_updated_at_snapshot == profile_updated_at:
        return existing
    ...
    if provider.provider_name == "mock":
        statement.statement_text = NOT_CONFIGURED_TEXT
        statement.provider = "mock"
    else:
        raw_text, provider_name = generate_statement_text(user, application.job)
        statement.statement_text = validate_statement_text(user, application.job, raw_text)
